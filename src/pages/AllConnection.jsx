...
const AllConnection = () => {
    ...
    const fetchConnections = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/clustertaskmanagment/connectionmanegment/${userId}`
        );
  ...
  const handleSendFriendRequest = async (receiverId) => {
    try {
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/clustertaskmanagment/connectionmanegment/send-request`, {
        senderId: userId,
        receiverId,
      });
  ...
  const handleAcceptRequest = async (connectionId, senderId) => {
    try {
      await axios.put(`${import.meta.env.VITE_BACKEND_URL}/clustertaskmanagment/connectionmanegment/update/${connectionId}`, {
        status: "accepted",
      });
  ...
  const handleRemoveFriend = async (connectionId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/clustertaskmanagment/connectionmanegment/remove/${connectionId}`);
